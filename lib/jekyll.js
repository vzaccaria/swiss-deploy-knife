...
(function(){
  var _, moment, fs, color, ref$, spawn, kill, __q, os, cl, ut, winston, dispOk, dispKo, disp, pdisp, pdeb, createPost, createContent, createLezione, createComunicazione, createEsercitazione, _module, split$ = ''.split;
  ...
  createContent = function(date, subCategory, tags){
    ...
    if (tags.length === 0) {
      nDate = subCategory + " del " + date.format('D/M');
    } else {
  ...
  createComunicazione = function(commaSeparatedTags){
    var tgs, ref$, name, content, po;
    if (commaSeparatedTags != null) {
      tgs = split$.call(commaSeparatedTags, ',');
    } else {
    ...
    ref$ = createContent(moment(), 'comunicazione', tgs), name = ref$.name, content = ref$.content;
    po = createPost(name, content, 'infob', moment(), tgs);
    return po;
  };
  ...
  _module = function(){
    ...
    iface = {
      ...
      createComunicazione: createComunicazione
    };
    return iface;
  };
