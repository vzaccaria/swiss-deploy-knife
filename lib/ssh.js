...
(function(){
  var _, moment, color, __q, os, cl, ut, winston, dispOk, dispKo, disp, pdisp, pdeb, lftpScript, src, otm, cwd, additionalSshParameters, surlGet, lftpScriptPut, lftpScriptGet, lftp_opts, lftpScriptMirror, _module, split$ = ''.split;
  ...
  cwd = process.cwd();
  additionalSshParameters = ["-o", "StrictHostKeyChecking=no", "-o", "LogLevel=quiet"];
  ...
  _module = function(){
    ...
    append = function(value, options){
      ...
      combine = function(it){
        ...
        res = (function(){
          switch (false) {
          case options.filterAll == null:
            return res;
          case options.first == null:
            return _.first(res, options.first);
          default:
            return _.first(res, 25);
          }
        }());
        res = _.filter(res, function(it){
          return !_.isNull(it);
        });
        return res;
      };
    ...
    openTerminal = function(address){
      var d, args, command, a, ch2, us, url, pw, cc;
      d = __q.defer();
      if (address.access === 'ssh') {
        args = [address.username + "@" + address.hostname, "-p", address.port + ""];
        args = args.concat(additionalSshParameters);
        command = (function(){
    ...
    save = function(what, options){
      ...
      if (options['in'] == null || options.to == null) {
        ...
        if (node !== 'local') {
          ...
          dd = setupTemporaryDirectory();
          fs.writeFile(dd + "/temp.json", JSON.stringify(what, null, 4), function(err){
    ...
    iface = {
      ...
      additionalSshParameters: additionalSshParameters
    };
    return iface;
  };
