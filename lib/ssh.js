...
(function(){
  ...
  _module = function(){
    ...
    append = function(value, options){
      var tmp, e, conn, shutdownOk, shutdownFail;
      tmp = setupTemporaryDirectory();
      get(local, remotePath, tmp + "/temp.json").then(function(){
        var data;
        data = fs.readFileSync(tmp + "/temp.json", 'utf-8');
        data = JSON.parse(data);
        if (!_.isArray(data)) {
          throw "Expecting remote json array";
        } else {
          data = data.concat([value]);
          return save(data, remotePath);
        }
      });
      e = __q.defer();
