(function(){
  var _, moment, fs, color, os, shelljs, table, __q, winston, invokeActions, ref$, mountTunnel, unmountTunnel, printEnv, getInterestedNodes, dispOk, dispKo, disp, pdisp, pdeb, psetup, name, description, author, year, src, otm, cwd, usageString, optimist, argv, ff, nodes, namespace, e, ok, ko, copy, original, current, i$, len$, n;
  ...
  getInterestedNodes = require('../lib/sk-lib').getInterestedNodes;
  dispOk = function(){
  ...
  }).boolean('l').boolean('d').boolean('s').argv;
  if (argv.help) {
    optimist.showHelp();
  }
  if (argv.file == null) {
